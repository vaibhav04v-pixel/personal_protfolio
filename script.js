...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (heroTitle) {
        ...
        setTimeout(() => {
            ...
            let i = 0;
            const highlightStart = textContent.indexOf('Vaibhav Velumuri');
            const beforeHighlight = textContent.substring(0, highlightStart);
            const highlightText = 'Vaibhav Velumuri';
            const afterHighlight = textContent.substring(highlightStart + highlightText.length);
            
            function typeWithHighlight() {
                if (i < beforeHighlight.length) {
                    heroTitle.innerHTML += beforeHighlight.charAt(i);
                    i++;
                    setTimeout(typeWithHighlight, 50);
                } else if (i === beforeHighlight.length) {
                    // Add the highlighted span
                    heroTitle.innerHTML += '<span class="highlight">' + highlightText + '</span>';
                    i += highlightText.length;
                    setTimeout(typeWithHighlight, 50);
                } else if (i < beforeHighlight.length + highlightText.length + afterHighlight.length) {
                    const afterIndex = i - beforeHighlight.length - highlightText.length;
                    if (afterIndex < afterHighlight.length) {
                        heroTitle.innerHTML += afterHighlight.charAt(afterIndex);
                    }
                    i++;
                    setTimeout(typeWithHighlight, 50);
                }
...
themeToggle.addEventListener('click', () => {
    ...
    // Show notification
    const themeText = body.classList.contains('dark-mode') ? 'Dark' : 'Light';
    showNotification(`${themeText} mode activated!`, 'info');
});
